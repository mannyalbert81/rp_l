...
class EntidadBase{
    ...
    public function convertir_a_letras($xcifra)
    {
        
        
        
        $monto = number_format((float)$xcifra, 2, ',', '.');
                ...
                if (substr(trim($xcadena), -7, 7) == "ILLONES") // si la cadena obtenida en MILLONES o BILLONES, entoncea le agrega al final la conjuncion DE
                    ...
                    if (trim($xaux) != "") {
                        switch ($xz) {
                            case 0:
                                if (trim(substr($XAUX, $xz * 6, 6)) == "1")
                                    $xcadena.= "UN BILLON DE DÓLARES DE LOS ESTADOS UNIDOS DE AMERICA (USD $$monto)";
                                    else
                                        $xcadena.= " BILLONES DE DÓLARES DE LOS ESTADOS UNIDOS DE AMERICA (USD $$monto)";
                                        break;
                            case 1:
                                if (trim(substr($XAUX, $xz * 6, 6)) == "1")
                                    $xcadena.= "UN MILLON DE DÓLARES DE LOS ESTADOS UNIDOS DE AMERICA (USD $$monto)";
                                    else
                                        $xcadena.= " MILLONES DE DÓLARES DE LOS ESTADOS UNIDOS DE AMERICA (USD $$monto)";
                                        break;
                            case 2:
                                if ($xcifra < 1) {
                                    $xcadena = "CERO DÓLARES CON $xdecimales/100 DE LOS ESTADOS UNIDOS DE AMERICA (USD $$monto)";
                                }
                                if ($xcifra >= 1 && $xcifra < 2) {
                                    $xcadena = "UN DÓLAR CON $xdecimales/100 DE LOS ESTADOS UNIDOS DE AMERICA (USD $$monto)";
                                }
                                if ($xcifra >= 2) {
                                    $xcadena.= " CON $xdecimales/100 DÓLARES DE LOS ESTADOS UNIDOS DE AMERICA (USD $$monto)"; //
                                }
                                break;
                        } // endswitch ($xz)
                    } // ENDIF (trim($xaux) != "")
    ...
    public function devuelve_seguro_desgravamen($saldo_capital){
    	...
    	echo "Seguro Desgravamen: " . round($seguro,2);
    ...
    public function devuelve_interes_ord_x_capital($_dias, $_capital, $_tasa) {
	 	...
	 	$_tasa_interes = $_tasa / 100;
    	$interes_mensual = $_tasa_interes / 12;
	    $interes_diario=$interes_mensual /30;
	 ...
	 //   echo "Interes diario: " . $interes_diario;
	    echo "Interes Cncesion: " . $interes_concesion;
