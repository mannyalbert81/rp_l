...
function loadDetalleFactura(pagina=1){
	...
	let $comprobanteId = $('#id_comprobante');
	
	$.ajax({
		type: "POST",
        url: 'index.php?controller=MovimientosInv&action=detalleFactura',
        data: {page:pagina,comprobante_id:$comprobanteId.val()},
    	beforeSend: function(objeto){
    		/*$("#resultados").html("Mensaje: Cargando...");*/
    	  },
	}).done(function(data){
		$('#detalle_factura').html(data);
		generaTabla("tabla_temporal");
	}).fail(function(){
...
$("#mod_id_grupo").on("change",function(){
	
	let $ddlGrupo = $(this);
	let $codigoProducto = $("#mod_codigo_producto");
	$codigoProducto.val('');
